 ...
 async delete(){
  ...
  async getdataa(choo?: boolean) {
    ...
    let data = await this.rest.PostDataToAPI<any[]>({ip:this.listMKV8002,  startdate: this.startdate, enddate: this.enddate,style:2 }, 'KTX0050/Getall').toPromise()
      ////console.log(data)
      let arrUser_ID=[...new Set(data.map(x=>x.User_ID))]
      ////console.log(arrUser_ID)
      arrUser_ID.map((x,index)=>{
        let list=data.filter(c=>c.User_ID===x)
        this.listMKV9999.push(list[0])
        this.listMKV9999[index].count=[...new Set(list.map(x=>x.ngay))].length;
        let max=0;
        list.push({ngay:this.enddate.substring(0,this.enddate.indexOf(' ')).replace('-','').replace('-','').replace('-','')})
        list.unshift({ngay:this.startdate.substring(0,this.startdate.indexOf(' ')).replace('-','').replace('-','').replace('-','')})
        list.map(h=>h.ngay=h.ngay-0)
        list.sort((a,b)=>a.ngay<b.ngay?-1:a.ngay>b.ngay?1:0)
        //console.log(list)
        
        list.map((g,index)=>{
          if(index+1<list.length)
          if(max<(list[index+1].ngay-0)-(g.ngay-0))max=(list[index+1].ngay-0)-(g.ngay-0)
        })
        this.listMKV9999[index].step=max
      })
    ...
    // if(x.MKV9999!=null){
    //   let kl=this.listMKV9998.filter(c=>c.phong_id===x.MKV9999.phong_id).map(c=>c.bophan_ten)
    //   x.MKV9999.thetu_id=kl.length>0?kl[0]:''
    // }
    // })
  ...
  test(){
    ...
    //console.log(this.enddate.substring(0,this.enddate.indexOf(' ')).replace('-','').replace('-','').replace('-',''))
  }
}
